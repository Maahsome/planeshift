# Work 08: Implement Plane API — Custom Properties

Use this file as the implementation prompt for the Custom Properties slice of `planeshift` (Jira ticket PSFT-2).

## Prompt

You are an AI coding agent working in the `planeshift` repository. Read `AGENTS.md`, `CONSTITUTION.md`, `ARCHITECTURE.md`, `LOCAL_BUILD.md`, `work/README.md`, and the foundation prompt before editing. This slice depends on [`07-work-item-types.md`](./07-work-item-types.md).

Implement the Custom Properties resource completely. The scope is the 5 HTTP operations listed below. Do not implement another resource or invent behavior not present in the linked Plane documentation.

### Documentation to implement

- [Custom Properties overview](https://developers.plane.so/api-reference/issue-types/properties/overview.md)
- [Create a custom property](https://developers.plane.so/api-reference/issue-types/properties/add-property.md)
- [List custom properties](https://developers.plane.so/api-reference/issue-types/properties/list-properties.md)
- [Retrieve a custom property](https://developers.plane.so/api-reference/issue-types/properties/get-property-details.md)
- [Update a custom property](https://developers.plane.so/api-reference/issue-types/properties/update-property.md)
- [Delete a custom property](https://developers.plane.so/api-reference/issue-types/properties/delete-property.md)

Implement custom-property definition CRUD for work item types, including property type, relation type, defaults, validation rules, required/active/multi flags, and nullable/dynamic values.
