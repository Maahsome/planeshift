# Work 09: Implement Plane API — Custom Property Values

Use this file as the implementation prompt for the Custom Property Values slice of `planeshift` (Jira ticket PSFT-2).

## Prompt

You are an AI coding agent working in the `planeshift` repository. Read `AGENTS.md`, `CONSTITUTION.md`, `ARCHITECTURE.md`, `LOCAL_BUILD.md`, `work/README.md`, and the foundation prompt before editing. This slice depends on [`08-custom-properties.md`](./08-custom-properties.md), [`04-work-items.md`](./04-work-items.md).

Implement the Custom Property Values resource completely. The scope is the 5 HTTP operations listed below. Do not implement another resource or invent behavior not present in the linked Plane documentation.

### Documentation to implement

- [Custom Property Values overview](https://developers.plane.so/api-reference/issue-types/values/overview.md)
- [Add custom property values](https://developers.plane.so/api-reference/issue-types/values/add-property-values.md)
- [List all custom property values](https://developers.plane.so/api-reference/issue-types/values/list-property-values.md)
- [Get property value](https://developers.plane.so/api-reference/issue-types/values/get-property-value-detail.md)
- [Update property value](https://developers.plane.so/api-reference/issue-types/values/update-property-value.md)
- [Delete property value](https://developers.plane.so/api-reference/issue-types/values/delete-property-value.md)

Implement value CRUD and correctly model values that may be strings, arrays, booleans, numbers, UUIDs, or other JSON shapes according to the live endpoint schema.
