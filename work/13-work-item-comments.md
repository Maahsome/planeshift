...
Use this file as the implementation prompt for the Work Item Comments slice of `planeshift` (Jira ticket PSFT-2).
...
Implement the Work Item Comments resource completely. The scope is the 5 HTTP operations listed below. Do not implement another resource or invent behavior not present in the linked Plane documentation.
...
Implement comment CRUD while preserving HTML, stripped text, JSON content, attachments, access level, external identifiers, actor, and edit timestamps.
