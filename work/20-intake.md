...
Use this file as the implementation prompt for the Intake slice of `planeshift` (Jira ticket PSFT-2).
...
Implement the Intake resource completely. The scope is the 5 HTTP operations listed below. Do not implement another resource or invent behavior not present in the linked Plane documentation.
...
Implement intake work-item CRUD and preserve the fields needed to review, accept, reject, or otherwise process an intake item as documented. Keep intake identifiers distinct from normal project work-item identifiers.
