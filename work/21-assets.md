...
Use this file as the implementation prompt for the Assets slice of `planeshift` (Jira ticket PSFT-2).
...
Implement the Assets resource completely. The scope is the 6 HTTP operations listed below. Do not implement another resource or invent behavior not present in the linked Plane documentation.
...
Implement user- and workspace-scoped asset metadata plus upload-credential creation. Stream file data through the returned presigned form when required, preserve all returned upload fields, and keep asset cleanup explicit.
