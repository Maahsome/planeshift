# Work 37: Implement Plane API — Release Changelog

Use this file as the implementation prompt for the Release Changelog slice of `planeshift` (Jira ticket PSFT-2).
...
Implement the Release Changelog resource completely. The scope is the 2 HTTP operations listed below. Do not implement another resource or invent behavior not present in the linked Plane documentation.

### Documentation to implement

- [Retrieve a release changelog](https://developers.plane.so/api-reference/release/get-release-changelog.md)
- [Update a release changelog](https://developers.plane.so/api-reference/release/update-release-changelog.md)

Implement read/update changelog operations without regenerating or normalizing content locally; the server remains authoritative for the changelog representation.
