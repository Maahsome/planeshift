...
Use this file as the implementation prompt for the Workspace Invitations slice of `planeshift` (Jira ticket PSFT-2).

## Prompt

You are an AI coding agent working in the `planeshift` repository. Read `AGENTS.md`, `CONSTITUTION.md`, `ARCHITECTURE.md`, `LOCAL_BUILD.md`, `work/README.md`, and the foundation prompt before editing. This slice depends on [`00-api-client-foundation.md`](./00-api-client-foundation.md), [`47-members.md`](./47-members.md).

Implement the Workspace Invitations resource completely. The scope is the 5 HTTP operations listed below. Do not implement another resource or invent behavior not present in the linked Plane documentation.
...
Implement invitation CRUD, preserving email/role/status/token/expiry behavior as documented without exposing invitation secrets in logs or default output.
