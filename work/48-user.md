...
Use this file as the implementation prompt for the User slice of `planeshift` (Jira ticket PSFT-2).
...
Implement the User resource completely. The scope is the 1 HTTP operation listed below. Do not implement another resource or invent behavior not present in the linked Plane documentation.
...
Implement the authenticated-current-user lookup as a read-only identity operation. Do not add local user persistence or an alternate authentication flow.
